...
// Home route: if logged in -> dashboard, else render public home
app.get("/", async (req, res) => {
  ...
  // Public home page (same behaviour as before)
  try {
    ...
    const processedAthletes = (athletes || []).map(athlete => {
      try { if (athlete.achievements) athlete.achievements = JSON.parse(athlete.achievements); } catch(e) {}
      try { if (athlete.personal_records) athlete.personal_records = JSON.parse(athlete.personal_records); } catch(e) {}
      return athlete;
    });
